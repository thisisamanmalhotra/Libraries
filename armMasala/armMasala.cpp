...
void motor::serialRun(int p1,int p2)
{
 if(p1==0 && p2==1)
  Serial1.write(80);
 else if(p1==1 && p2==0)
...
void armMasala::boot(int m1Pin[4],int m2Pin[4],int m3Pin[4])
{
  LA1.setPins(m1Pin);
  LA2.setPins(m2Pin);
  pinMode(m3Pin[3],INPUT);
}

void armMasala::gotoPot(double la1,double la2,double tt,double vel)
{
...
void armMasala::gotoSemiDir(double d1,double d2,double d3,double vel)
{
 ...
 if     (dZdt>0)      TT.serialRun(1,0);
 else if(dZdt<0)	  TT.serialRun(0,1);
 else                 TT.serialRun(0,0);
 /*
 int val=64;
 int rawval=TT.velocity*dZdt;
 val=map(rawval,-100,100,0,127);  
 Serial1.write(val);
 //sabertooth();
 */
}
